...
export default function RealTimeDashboard() {
  ...
  const handleExportRevenueToExcel = () => {
    ...
    XLSX.writeFile(workbook, `stashhouse-revenue-${rangeSuffix}.xlsx`);
  };

  const handleCopyLink = () => {
    navigator.clipboard.writeText("https://my-ecommerce-app-theta-rose.vercel.app/");
    setCopied(true);
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#f8f9fa] flex items-center justify-center gap-2 font-bold text-gray-500 text-xs tracking-wider uppercase">
        <Loader2 size={18} className="animate-spin text-indigo-600" /> Syncing stash house Live Central Matrix...
      </div>
    );
  }
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-black text-gray-900 tracking-tight">
              {greeting.text}, Stash House! {greeting.emoji}
            </h1>
            <p className="text-xs font-semibold text-gray-400 mt-0.5">Trust the process! 🌊</p>
          </div>
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
          <a href="https://my-ecommerce-app-theta-rose.vercel.app/" target="_blank" rel="noopener noreferrer" className="px-4 py-2 border border-indigo-600 text-indigo-600 rounded-xl text-xs font-bold transition-all hover:bg-indigo-50 flex items-center gap-1.5">
